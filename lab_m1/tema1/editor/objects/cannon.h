...
class Cannon : public Placed {
public:

    Cannon(std::vector<Mesh*> meshes, std::vector<glm::mat4> offsets, glm::mat4 gridOffset, float x, float y, float z)
        : Placed(meshes, offsets, gridOffset, x, y, z) {
        type = 2;
    }

    Cannon(Mesh* mesh, float x, float y, float z)
        : Placed(mesh, x, y, z) {
		type = 2;
	}
